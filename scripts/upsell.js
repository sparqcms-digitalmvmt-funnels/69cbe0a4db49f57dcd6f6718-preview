...
const UPSELL_NEXT_PAGE_SLUG = "old-upsell";
    ...
    const response = await fetch(
      `https://app-cms-api-proxy-staging-001.azurewebsites.net/vrio/order_offers`,
      {
        ...
        body: JSON.stringify({
          ...
          order_id: lastOrderId,
          pageId: "4mzMWJtD42VISgT_NNg7aNc10Ovz7pDMBKm-19oaRBBUlvdtypbSHWl2VR5KSWbl"
        })
      }
    );
...
const processUpsell = async () => {
  ...
  try {
    const orderData = JSON.parse(sessionStorage.getItem("orderData"));
    orderData.pageId = "4mzMWJtD42VISgT_NNg7aNc10Ovz7pDMBKm-19oaRBBUlvdtypbSHWl2VR5KSWbl";
    const lastOrderId = sessionStorage.getItem("cms_oid");
